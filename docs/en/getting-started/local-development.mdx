...
Local development has three parts: **dependencies** (PostgreSQL and Redis, plus FFmpeg for canvas video composition), the **server** (Spring Boot on `18080`), and the **web app** (Next.js on `15555`). In development the web app proxies `/api/v1/**` to the server, so the two ports must line up.
...
| `FRONTEND_BASE_URL` | `http://localhost:15555` | Required; used to build password reset links |
| `SERVER_PORT` | `18080` | Server listen port; the fallback default in the config is already `18080`, matching what the web app requests |
...
```dotenv
NEXT_PUBLIC_SERVER_URL=http://127.0.0.1:18080
```
...
`pnpm dev` runs `next dev --turbo -H 0.0.0.0 -p 15555`; open `http://localhost:15555`.

## 5. Verify

- Web app: `http://localhost:15555` shows the sign-in page.
- Server: `http://127.0.0.1:18080/api/v1/health` returns `{"code":200,"data":"OK"}`.
- API reference: `http://127.0.0.1:18080/swagger/index.html`.
...
- **Port mismatch**: the server's `SERVER_PORT` and `NEXT_PUBLIC_SERVER_URL` in `web/.env.local` must point at the same port; after changing one, remember to change the other.
- **Dependencies not ready**: check with `pg_isready -h 127.0.0.1 -p 15432` and `redis-cli -h 127.0.0.1 -p 16379 ping`.
