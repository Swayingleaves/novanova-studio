...
| `POSTGRES_USERNAME` / `POSTGRES_PASSWORD` | `postgres` / `123456` | Database credentials. Port 15432 is published on every host interface by default, so a public machine must change the password or add a firewall rule |
| `REDIS_PASSWORD` | Empty (no auth) | Port 16379 is published the same way; set a password for any public deployment |
| `CORS_ALLOWED_ORIGIN_PATTERNS` | Includes `localhost:15555` and `www.novanovastudio.cn` | Replace with your real origins, comma-separated |
...
- `POSTGRES_PORT` / `REDIS_PORT` / `SERVER_PORT` / `WEB_PORT`: these are the **host-side** published ports, defaulting to `15432` / `16379` / `18080` / `15555`. Container ports are fixed (5432 / 6379 / 8080 / 5550), so only change these on a port conflict.
...
```bash
# Minimal: open http://127.0.0.1:15555 afterwards
...
curl http://127.0.0.1:18080/api/v1/health # expect {"code":200,"data":"OK"}
