    ...
    @Override public List<AgentTool> tools() {
        return List.of(
            new AgentTool("generate_video",
                "根据文字描述生成视频。prompt 为用户输入的描述，所有参数（videoGenerationMode、seconds、size、resolution、quality、model、watermark）均必须严格从用户设置中提取，不得自行修改。",
                JSON.parseObject("{\"type\":\"object\",\"properties\":{\"prompt\":{\"type\":\"string\"},\"videoGenerationMode\":{\"type\":\"string\",\"enum\":[\"text-to-video\",\"image-to-video\",\"reference-to-video\",\"first-last-frame-to-video\"],\"description\":\"必须严格使用用户设置中的视频生成模式\"},\"seconds\":{\"type\":\"string\",\"description\":\"从用户设置的 [时长=X] 中提取 X，仅数字不含单位\"},\"size\":{\"type\":\"string\",\"description\":\"从用户设置的 [尺寸=X] 中提取\"},\"resolution\":{\"type\":\"string\",\"description\":\"从用户设置的 [分辨率=X] 中提取\"},\"quality\":{\"type\":\"string\",\"enum\":[\"low\",\"medium\",\"high\"],\"description\":\"视频质量等级，根据分辨率自动判断：≤480→low，720→medium，≥1080→high\"},\"model\":{\"type\":\"string\",\"description\":\"必须严格使用用户设置 [视频模型=X] 中的 X 值，不得自行选择\"},\"watermark\":{\"type\":\"boolean\",\"default\":false}},\"required\":[\"prompt\",\"model\",\"videoGenerationMode\"],\"additionalProperties\":false}"),
                false),
            new AgentTool("edit_video",
                "基于参考素材生成视频。参考素材严格使用用户上传内容，必须保留用户选择的生成模式和参考素材语义；不支持时直接返回错误，不得降级为文生视频。所有参数（videoGenerationMode、model、size、resolution、seconds、quality、watermark）均必须严格从用户设置中提取，不得自行修改。",
                JSON.parseObject("{\"type\":\"object\",\"properties\":{\"prompt\":{\"type\":\"string\"},\"videoGenerationMode\":{\"type\":\"string\",\"enum\":[\"text-to-video\",\"image-to-video\",\"reference-to-video\",\"first-last-frame-to-video\"],\"description\":\"必须严格使用用户设置中的视频生成模式\"},\"seconds\":{\"type\":\"string\",\"description\":\"从用户设置的 [时长=X] 中提取 X，仅数字不含单位\"},\"size\":{\"type\":\"string\",\"description\":\"从用户设置的 [尺寸=X] 中提取\"},\"resolution\":{\"type\":\"string\",\"description\":\"从用户设置的 [分辨率=X] 中提取\"},\"quality\":{\"type\":\"string\",\"enum\":[\"low\",\"medium\",\"high\"],\"description\":\"视频质量等级，根据分辨率自动判断：≤480→low，720→medium，≥1080→high\"},\"model\":{\"type\":\"string\",\"description\":\"必须严格使用用户设置 [视频模型=X] 中的 X 值，不得自行选择\"},\"watermark\":{\"type\":\"boolean\",\"default\":false}},\"required\":[\"prompt\",\"model\",\"videoGenerationMode\"],\"additionalProperties\":false}"),
                false),
