...
export const ImageNode = memo(function ImageNode({ data, selected }: NodeProps<Node<CanvasImageNode>>) {
  ...
  if (data.grouping.isRoot) {
    ...
    const statusText = isError
      ? data.execution.errorMessage || "生成失败"
      : data.execution.phase === "succeeded"
        ? `已完成 ${totalCount}/${totalCount}`
        : `生成中 ${completedCount}/${totalCount}`;

    return (
      <>
        <NodeResizer minWidth={280} minHeight={180} keepAspectRatio={false} isVisible={selected} lineStyle={{ borderColor: theme.node.activeStroke }} handleStyle={{ borderColor: theme.node.activeStroke, backgroundColor: theme.node.panel }} onResizeEnd={(_, params) => { actions.onResize?.(data.id, params.width, params.height, { x: params.x, y: params.y }); }} />
        <NodeHoverSurface
          nodeId={data.id}
        ...
        >
          <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-[inherit] p-4">
            <div className="flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <div className="truncate text-sm font-semibold" style={{ color: theme.node.text }}>图片生成</div>
              ...
              <span className="max-w-[130px] truncate text-[11px] font-medium" style={{ color: isError ? "#ef4444" : theme.node.muted }} title={statusText}>{statusText}</span>
            </div>
            ...
            <div className="mt-3 grid grid-cols-[44px_minmax(0,1fr)] gap-x-3 gap-y-1.5 border-t pt-3 text-[11px]" style={{ borderColor: theme.node.stroke }}>
              ...
              <span className="truncate text-right" style={{ color: theme.node.text }}>{data.generation.model || "未配置"}</span>
              <span style={{ color: theme.node.muted }}>尺寸</span>
              <span className="text-right" style={{ color: theme.node.text }}>{data.generation.size || "自动"}</span>
            </div>
          ...
          {data.grouping.childIds.length ? <button
            type="button"
            ...
            onMouseEnter={(event) => { event.currentTarget.style.background = theme.node.fill; event.currentTarget.style.color = theme.node.text; }}
            onMouseLeave={(event) => { event.currentTarget.style.background = theme.node.panel; event.currentTarget.style.color = theme.node.muted; }}
            onClick={(event) => { event.stopPropagation(); actions.onToggleBatch?.(data.id); }}
          >
  ...
  return (
    <>
      <NodeResizer minWidth={220} minHeight={160} keepAspectRatio={!data.frame.freeResize} isVisible={selected} lineStyle={{ borderColor: theme.node.activeStroke }} handleStyle={{ borderColor: theme.node.activeStroke, backgroundColor: theme.node.panel }} onResizeEnd={(_, params) => { actions.onResize?.(data.id, params.width, params.height, { x: params.x, y: params.y }); }} />
      <NodeHoverSurface
        nodeId={data.id}
      ...
      >
        ...
        >
          {data.grouping.rootId ? (
            <span
              className="absolute left-3 top-3 z-10 text-[11px] font-medium opacity-70"
              style={{ color: "rgba(255, 255, 255, 0.9)", textShadow: "0 1px 2px rgba(0, 0, 0, 0.65)" }}
            >
              {data.title}
            </span>
          ) : null}
  ...
  return (
    <div ref={stackRef} aria-hidden className="pointer-events-none absolute left-[calc(100%+64px)] top-[calc(50%-40px)] h-20 w-32">
      {previews.slice(0, 3).map((preview, index) => (
        ...
        >
          <img src={preview.source} alt="" className="h-full w-full object-cover" draggable={false} />
        </span>
      ))}
    </div>
  );
